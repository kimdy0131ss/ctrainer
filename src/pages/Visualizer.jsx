...
const CONCEPTS = {
  bfs: {
    ...
    steps: [
      { icon: '1', text: '시작 노드를 큐에 넣고, 방문 처리한다.' },
      { icon: '2', text: '큐에서 노드를 하나 꺼낸다.' },
      { icon: '3', text: '해당 노드의 인접 노드 중 방문하지 않은 것을 모두 큐에 넣고 방문 처리한다.' },
      { icon: '4', text: '큐가 빌 때까지 2~3을 반복한다.' },
    ],
    props: [
  ...
  dfs: {
    ...
    steps: [
      { icon: '1', text: '시작 노드를 스택에 넣고, 방문 처리한다.' },
      { icon: '2', text: '스택에서 노드를 하나 꺼낸다.' },
      { icon: '3', text: '해당 노드의 방문하지 않은 인접 노드를 스택에 넣고 방문 처리한다.' },
      { icon: '4', text: '스택이 빌 때까지 2~3을 반복한다.' },
    ],
    props: [
      { label: '시간 복잡도', value: 'O(V + E)', desc: '모든 정점·간선을 한 번씩 탐색' },
  ...
  dijkstra: {
    ...
    steps: [
      { icon: '1', text: '시작 노드의 비용을 0, 나머지는 무한대(∞)로 초기화한다.' },
      { icon: '2', text: '비용이 가장 낮은 미확정 노드를 우선순위 큐에서 꺼낸다.' },
      { icon: '3', text: '인접 노드까지의 새 비용 = 현재 비용 + 간선 가중치를 계산한다.' },
      { icon: '4', text: '새 비용이 기존보다 낮으면 업데이트(Relaxation)하고 큐에 넣는다.' },
      { icon: '5', text: '큐가 빌 때까지 2~4를 반복한다.' },
    ],
    props: [
...
const SORT_CONCEPTS = {
  bubble: {
    ...
    steps: [
      { icon: '1', text: '첫 번째 원소부터 인접한 두 원소를 비교한다.' },
      { icon: '2', text: '앞 원소가 뒤 원소보다 크면 교환한다.' },
      { icon: '3', text: '한 번의 순회가 끝나면 가장 큰 원소가 맨 뒤에 확정된다.' },
      { icon: '4', text: '정렬된 부분을 제외하고 1~3을 반복한다.' },
    ],
    props: [
  ...
  selection: {
    ...
    structure: 'In-place', structureDesc: '추가 메모리 없이 배열 내에서 정렬',
    steps: [
      { icon: '1', text: '정렬되지 않은 부분의 첫 번째를 최솟값 후보로 설정한다.' },
      { icon: '2', text: '나머지 원소와 비교하여 더 작으면 최솟값 후보를 교체한다.' },
      { icon: '3', text: '순회가 끝나면 최솟값을 정렬되지 않은 부분 첫 위치와 교환한다.' },
      { icon: '4', text: '정렬 범위를 한 칸씩 줄여가며 반복한다.' },
    ],
    props: [
  ...
  insertion: {
    ...
    structure: 'In-place', structureDesc: '추가 메모리 없이 배열 내에서 정렬',
    steps: [
      { icon: '1', text: '두 번째 원소부터 시작해 현재 원소를 key로 저장한다.' },
      { icon: '2', text: 'key보다 큰 이전 원소들을 한 칸씩 오른쪽으로 이동한다.' },
      { icon: '3', text: '빈 자리에 key를 삽입한다.' },
      { icon: '4', text: '모든 원소에 대해 1~3을 반복한다.' },
    ],
    props: [
  ...
  merge: {
    ...
    steps: [
      { icon: '1', text: '배열을 절반으로 나눈다.' },
      { icon: '2', text: '왼쪽 절반을 재귀적으로 병합 정렬한다.' },
      { icon: '3', text: '오른쪽 절반을 재귀적으로 병합 정렬한다.' },
      { icon: '4', text: '정렬된 두 절반을 비교하며 병합한다.' },
    ],
    props: [
  ...
  quick: {
    ...
    steps: [
      { icon: '1', text: '피벗(기준값)을 선택한다. (보통 마지막 원소)' },
      { icon: '2', text: '피벗보다 작은 원소는 왼쪽, 큰 원소는 오른쪽으로 이동한다.' },
      { icon: '3', text: '피벗을 올바른 위치에 놓는다. (피벗 위치 확정)' },
      { icon: '4', text: '피벗 양쪽을 재귀적으로 1~3 반복한다.' },
    ],
    props: [
